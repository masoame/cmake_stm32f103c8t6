...
namespace rfid {
    ...
    rc522::rc522(SPI_HandleTypeDef* const hspi, GPIO_TypeDef* const sda_port,const uint16_t sda_pin,GPIO_TypeDef* const rst_port,const uint16_t rst_pin)
      : m_hspi(hspi), m_sda_port(sda_port), m_sda_pin(sda_pin), m_rst_port(rst_port), m_rst_pin(rst_pin)
    {
      	this->RC522_SDA_LOW();
                    ...
                    unsigned int  *pOutLenBit                       )
    {
      ...
      this->SetBitMask(FIFOLevelReg,0x80)                 ; // 清空FIFO 
      for(i=0; i<InLenByte; i++)
        this->WriteRawRC(FIFODataReg,pInData[i])          ; // 数据写入FIFO 
      this->WriteRawRC(CommandReg, Command)               ; // 命令写入命令寄存器
      if(Command == PCD_TRANSCEIVE)
        this->SetBitMask(BitFramingReg,0x80)              ; // 开始发送     
      i = 6000                                      ; //根据时钟频率调整，操作M1卡最大等待时间25ms
      do 
      {
      ...
      this->SetBitMask(ControlReg,0x80)                   ;// stop timer now
      this->WriteRawRC(CommandReg,PCD_IDLE)               ; 
      return status;
    }
    ...
    std::string rc522::ReaderCard(void)
    {
      ...
      if(this->PcdRequest(PICC_REQALL,Temp)==MI_OK)	//选卡
      {
        if(Temp[0]==0x04&&Temp[1]==0x00)  
          ...
          result = "Unknown\r\n";
        if(PcdAnticoll(this->m_uid)==MI_OK){
